...
def _render_card(index: int, item: ScoredCandidate) -> str:
    ...
    tier_label = "已验证机会" if tier == "validated" else "新发现观察"
    tier_badge = (
        f'<span class="tier {tier}">{tier_label} · 置信 {confidence}</span>'
    )
...
def render_html(
    ...
    watch_count = total_selected - validated_count
    section_html = ""
    for segment, items in sections.items():
        positioned = list(enumerate(items, start=1))
        validated = [value for value in positioned if value[1].candidate.metadata.get("opportunity_tier") == "validated"]
        watch = [value for value in positioned if value[1].candidate.metadata.get("opportunity_tier") != "validated"]
        groups = []
        if validated:
            groups.append(
...
<style>
  *{{box-sizing:border-box}} body{{margin:0;background:#f3f5f7;color:#16202a;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","PingFang SC","Microsoft YaHei",sans-serif;line-height:1.55}}
  ...
  <header class="hero">
    ...
    <div class="health"><b>{validated_count} 个已验证机会</b> · {watch_count} 个新发现观察　|　{healthy} 个健康数据源 · {degraded} 降级 · {failed} 失败</div>
  ...
  <footer>
    ...
    <p><strong>方法：</strong>综合 1/3/7/14/30 日榜位与评价增速、市场扩散、新鲜度、增长置信度和可借鉴性；政府专属、高合规、博彩、成熟 IP、强网络效应与大型制作产品使用硬门槛排除。只借鉴需求、机制和商业模式，不复制名称、美术、代码或受保护内容。</p>
  </footer>
</main></body></html>"""
...
def render_text(
    ...
    for segment, items in sections.items():
        ...
        for index, item in enumerate(items, start=1):
            candidate = item.candidate
            tier = "已验证" if candidate.metadata.get("opportunity_tier") == "validated" else "观察"
            confidence = int(candidate.metadata.get("confidence_score", 0))
            lines.append(
